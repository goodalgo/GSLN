# -*- coding: utf-8 -*-
"""
Alternative training of semi supervised graph neural network and text model  半监督图神经网络和文本模型交替训练  SSGTA
"""
import argparse
import time
import numpy as np
...
from util_dual_attention import train_pro as train_pro_cross
...
if __name__ == '__main__':
    ...
    X,Y,M = load_sigir_pandas_multi_select('data/ndatas/node_feat_sigir_preload_multi_select.csv', std_size=std_size, sep='\t')
    ...
    num_worker = 0
    lr_graph = 0.0005
    lr_text =  0.0003     # 0.003 h和 batch_s=32搭配  0.0003和 batch_s=1搭配
    device = 3
    ...
    preload_graph = False
    load_fix_nid = False  #只要load_fix_nid 设置为True，save_nid这个就不用管
    save_nid = False
