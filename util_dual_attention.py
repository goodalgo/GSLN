...
# from models.SageconvDualAtt import SAGEConv_DA
...
class GraphSAGE_cross(nn.Module):
    def __init__(self, 
        ...
        self.feat_drop = 0.0  #我们这边的feat是文本向量，丢弃了没有意义
...
#             time.sleep(1)
            ...
            h, batch_input_labels, batch_input_labels_ori = layer(block, layer_args) #layer(block, batch_args)
        ...
        return h   
            ...
            #参考https://github.com/dmlc/dgl/blob/c560040f5f70cc4d538ccbcca5079f1f98c5888f/examples/pytorch/graphsage/model.py#L39
...
#             print(f"inference layer{l}:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::")
            ...
            dataloader = dgl.dataloading.NodeDataLoader(
                    g,
                    val_nid,#torch.arange(g.num_nodes()).to(g.device),
                    sampler,
                    batch_size = batch_s,
                    shuffle=True,
                    drop_last=False,
...
#                 print("y[output_nodes], by[output_nodes], by_ori[output_nodes]")
#                 print(y[output_nodes], by[output_nodes], by_ori[output_nodes])
            nfeat = y
...
#     optimizer = torch.optim.Adam(itertools.chain(graph_model.parameters(), fea_para.parameters()), lr=0.01)
#     loss_fun = nn.CrossEntropyLoss()
    ...
    for epoch in range(graph_epoch):
        ...
        # 验证一下模型的准确率
        if epoch % 1 == 0:
#             print(f"_________________EVLUATE Val epoch{epoch}:_______________")
#             val_acc = evaluate(graph_model, g, text_pred_feat, text_pred_labels, ori_labels, val_nid, val_mask, batch_s, num_worker, device)
    ...
    print("macrof1_list:", macrof1_list)
    window_max_macro = get_window_max(macrof1_list, 5)
    print("Graph 5轮 平均macro f1最好值为：", window_max_macro)
    
    print("microf1_list:", microf1_list)
    window_max_micro = get_window_max(microf1_list, 5)
    print("Graph 5轮 平均micro f1最好值为：", window_max_micro)
    
    print("acc_list:", acc_list)
    window_max_acc = get_window_max(acc_list, 5)
    print("Graph 5轮 平均acc f1最好值为：", window_max_acc)
    ...
    print("######################################feat_pred.shape##################################", feat_pred.shape)
...
def predict_text_tensor(X, Y, text_model, test_nid, val_nid, device, Graph_emb=None):   
    print("使用Text模型生成新的Text Tensor 和 label:")
...
#                 print("Graph_emb[i].shape, batch_g.shape", Graph_emb[i].shape, batch_g.shape)
...
#     text_feat_size = 768  #textCNN 输出的embedding大小
...
#     print(val_nid)
    ...
    # print("t#######################测试预加载的Text模型 val ############################")
    # val_predicted = (outputs[val_nid].data>0.5).long().cpu().numpy()
    # val_labels = ori_labels[val_nid].cpu()
    # print(metrics.classification_report(val_predicted, val_labels, digits=4))
    
    print("t#######################测试预加载的Text模型 test############################")
    
    test_predicted = (outputs[test_nid].data>0.5).long().cpu().numpy()
    test_labels = ori_labels[test_nid].cpu()
    print(metrics.classification_report(test_predicted, test_labels, digits=4))
...
#     print("test_mask, train_nid", test_mask, train_nid)
    train_mask = torch.BoolTensor(train_mask)
    val_mask = torch.BoolTensor(val_mask)
    test_mask = torch.BoolTensor(test_mask)
    
    print("len(train_nid),  len(val_nid),  len(test_nid)", len(train_nid), len(val_nid), len(test_nid), len(train_nid)+len(val_nid)+  len(test_nid))
    
    text_model = prepare_text_model('TextCNN_v4', n_classes)
    text_model.to(device)
    print(text_model)
#     print("text_model.convs[0].weight",text_model.convs[0].weight)
    ...
    # print("第0轮，未传入Graph_emb训练TextCNN")
...
#     print(text_model)
    ...
    for i in range(10):
        print(f"大迭代轮数：{i}\n")
        ############################################利用中心节点训练 Text模型，并inference所有节点############################################################
        print("传入Graph_emb训练TextCNN")  #text_model自动使用上一轮的text_model，不用重新加载
